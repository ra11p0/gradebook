...
function PersonIndex(props: Props) {
  ...
  const [personActiveClass, setPersonActiveClass] = useState<ClassResponse | undefined>(undefined);
  useEffect(() => {
    if (personGuid) {
    ...
    <LoadingScreen isReady={personName != "" && personSurname != ""}>
      <>
        ...
        <Routes>
          ...
          <Route path="/" element={<Overview schoolRole={personSchoolRole} personGuid={personGuid ?? ""} />} />
        </Routes>
      </>
    </LoadingScreen>
  );
