    ...
    stages {
        ...
        stage('migrate databases'){
            steps {
                ...
                sh "sudo docker exec mysql /usr/bin/mysqldump -u ${params.dbUid} --password="+params.dbPassword.plainText+" ${params.dbName} > dbBackups/${params.dbName}-${BUILD_TAG}-bak.sql --no-tablespaces"
            ...
            post{
                failure{
                    sh "cat dbBackups/${params.dbName}-${BUILD_TAG}-bak.sql | sudo docker exec -i mysql /usr/bin/mysql -u ${params.dbUid} --password="+params.dbPassword.plainText+"  ${params.dbName}"
                }
            }
        }
